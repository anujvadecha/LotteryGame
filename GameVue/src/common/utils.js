...
function print_div(res, claim = false, user = null) {
  document.getElementById("printdivcontent").innerHTML +=
    "<div style='font-family: sans-serif; font-weight: medium; font-size: 16px;'>Star Skill Game</div>";
  if (claim == false) {
    document.getElementById("printdivcontent").innerHTML +=
      "<div style='font-family: sans-serif; font-weight: medium; font-size: 16px;'>" +
      user +
      "</div>";
    ...
    document.getElementById(
      "printdivcontent"
    ).innerHTML += `<div style='font-family: sans-serif; font-weight: medium; font-size: 16px;'>Created date: ${getTimeZoneDate(
      new Date(res["created_at"])
    ).toLocaleDateString("en-IN")} ${convert_to_twelve_hour_clock(
      new Date(res["created_at"])
    )} </div>`;
  }
  document.getElementById(
    "printdivcontent"
  ).innerHTML += `<div style='font-family: sans-serif; font-weight: medium; font-size: 16px;'>Draw date: ${getTimeZoneDate(
    new Date(res["lottery"]["time"])
  ).toLocaleDateString("en-IN")} ${convert_to_twelve_hour_clock(
    new Date(res["lottery"]["time"])
  )} </div>`;
  ...
  if (claim === false) {
    document.getElementById(
      "printdivcontent"
    ).innerHTML += `<div style='font-family: sans-serif; font-weight: medium; font-size: 16px;'>Ticket set: </div>`;

    var ticket_details = "";
    // res["ticket_set"] = res["ticket_set"].map(ele=>{return ele.set_ticket}).sort()
    res["ticket_set"].map(res => {
      ticket_details += `${res["set_ticket"]}-${res["price"] *
        res["quantity"]},`;
    });

    document.getElementById("printdivcontent").innerHTML +=
      "<div style='font-family: sans-serif; font-weight: medium; font-size: 16px;'>" +
      ticket_details.slice(0, -1) +
      "</div>";

    document.getElementById(
      "printdivcontent"
    ).innerHTML += `<div style='font-family: sans-serif; font-weight: medium; font-size: 16px;'>Total quantity ${res["total_quantity"]}</div>`;

    document.getElementById(
      "printdivcontent"
    ).innerHTML += `<div style='font-family: sans-serif; font-weight: medium; font-size: 16px;'>Total price ${res["total_price"]}</div>`;
  }

  if (claim == true) {
    document.getElementById(
      "printdivcontent"
    ).innerHTML += `<div style='font-family: sans-serif; font-weight: medium; font-size: 16px;'>Total wins ${res["inflow"]}</div>`;
    document.getElementById(
      "printdivcontent"
    ).innerHTML += `<div style='font-family: sans-serif; font-weight: medium; font-size: 16px;'>Ticket: ${res["ticket_id"]}</div>`;
  }
  if (claim == false) {
    document.getElementById(
      "printdivcontent"
    ).innerHTML += `<div><svg id="barcode" style=""></svg></div>`;
